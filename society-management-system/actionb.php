...
if (isset($_POST['action'])) {
    ...
    if ($_POST['action'] == 'fetch_bills') {
        ...
        // Define the base query
        $query = "
    SELECT bills.id, bills.bill_title, flats.flat_number, flats.block_number, bills.amount, bills.month, bills.paid_amount, payment.added_on, bills.remaining_amount,  bills.paid_date, payment.payment_id, payment.bid, payment.amount AS payment_amount,payment.payment_status
    FROM $table
    JOIN flats ON flats.id = bills.flat_id
    LEFT JOIN payment ON payment.bid = bills.id
";
        ...
        foreach ($results as $row) {
            ...
            $sub_array[] = (isset($row['remaining_amount']) && $row['remaining_amount'] == 0) ? '<span class="badge bg-success">Paid</span>' : $row['remaining_amount'];
