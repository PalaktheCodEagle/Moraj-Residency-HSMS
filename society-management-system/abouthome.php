...
if (isset($_GET['action'], $_GET['id']) && $_GET['action'] == 'delete' && $_SESSION['user_role'] == 'admin') {
    ...
    $_SESSION['success'] = 'The About Data has been removed';
...
<div class="container-fluid px-4">
    <h1 class="mt-4">About the Society</h1>
    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item"><a href="dashboard.php" style="text-decoration:none;">Dashboard</a></li>
        <li class="breadcrumb-item active">About the Society</li>
    </ol>
    <?php
...
?>
    <div class="card">
        <div class="card-header" style="background-color:#4C0033;">
            <div class="row">
                <div class="col col-6">
                    <h5 class="card-title" style="color: #fff;">About the Society</h5>
                </div>
                <div class="col col-6">
                    <?php
if ($_SESSION['user_role'] == 'admin') {
    ?>
                    <div class="float-end"><a href="add_about.php" class="btn btn-success btn-sm"
...
// $sql = "SELECT about_text FROM about ORDER BY timestamp DESC";
// $stmt = $pdo->query($sql);
...
?>
    ...
    text-align: center;">Date</th>
                <th style="    color: white;
    text-align: center;">About the Society</th>
     <th style="    color: white;
...
if ($stmt->rowCount() > 0) {
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($_SESSION['user_role'] == 'admin') {
            echo "<button class='download_btn' style='
            background: #DC3545;
            padding: 4px 8px;
            border-radius: 4px;
            color: white;
            border:none;' onclick='confirmDelete($about_id)'>Delete</button>";
        }
...
?>
    ...
    function confirmDelete(aboutId) {
        if (confirm("Are you sure you want to delete this announcement?")) {
            ...
            window.location.href = `delete_about.php?id=${aboutId}`;
        }
    }
    </script>
</div>
